"""Menus contributed by the ``field_diagrams`` panel type.

Order and grouping mirror :mod:`spot_diagram.menus` (and the existing
render panels) so users see the same affordances in the same slots
across every evaluation panel.
"""
...
def build_menus(body: FieldDiagramBody, project: Project) -> List[QMenu]:
    ...
    act_refresh.setToolTip(
        "Recompute the field diagrams now, bypassing the edit-settle debounce."
    )
    ...
    act_sync.setToolTip(
        ...
        "and max field angle with the project's System Setup values."
    )
    ...
    act_reset.setToolTip(
        "Replace the current spec with the panel's default field range, "
        "wavelengths, and sampling. Triggers an immediate recompute."
    )
